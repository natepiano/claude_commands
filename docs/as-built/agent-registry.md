...
| `scripts/delegate/progress_history.py` | Cross-agent append-only plan-delegate event recorder and aggregator. Durable per-run JSONL lives under `~/.local/state/plan-delegate/runs/`; live state remains in the session directory. It renders separate project and phase progress sections with independent unchanged timers and calibrates phase estimates from completed-phase history. |
