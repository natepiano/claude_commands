...
| `scripts/delegate/implement.sh` / `review.sh` | `/plan:delegate`'s launchers: `<session_dir> [working_dir] [prompt_file] [task] [role_description]`; sub-task defaults `implementation` / `review`, prefixed to `delegate.<subtask>`. Write `impl_status` / `review_status` (`implementing\|implemented\|error`, `reviewing\|reviewed\|error`), provenance to `impl_agent` / `review_agent`, logs `impl_agent.log` / `review_agent.log`, shared `heartbeat.log`; background `agent_exec` in `write` / `readonly` mode, watch it, propagate its exit code. |
