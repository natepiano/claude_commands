...
- **Key files:**
  ...
  - `scripts/clean-fix/clean-fix.sh` — pipeline driver; `cf_load_stage_assignment` calls at lines 225-229; since Phase 8 the report render goes through `agent_exec cleanfix.report write` (guarded prompt file beside the run log, `report_render.txt` render log, WARN-and-continue).
  - `scripts/clean-fix/clean-fix-usage.sh` — usage screen; renders family/agent/effort columns since Phase 8; help text points at `/agent`.
  - `scripts/clean-fix/clean_fix_report_parse.py` — report parser; family-carried usage-limit parsing since Phase 8 (`AGENT_LIMIT_LINE_RE` greedy, `AgentLimit` dataclass, `<family>-usage-limit` reason codes); must stay basedpyright-clean.
  ...
  - `commands/clean_fix.md` — configure surface became a status view pointing at `/agent` in Phase 8 (done); enable/disable subcommands retained.
...
- **Invariants:**
  ...
  - `settings.json` carries an app-generated key reorder unrelated to this plan — it stays excluded from every checkpoint commit (standing practice since Phase 3; the out-of-scope guardrail protects its `model`/`statusLine` keys anyway).
...
- `agent_admin.sh <function>.<subtask> <agent>[:<effort>]` — edit one row of the *active* set for that function: validate agent ∈ `[<family>.agents]` and effort ∈ that agent's list, then awk-rewrite the row in place; invalid input leaves the file untouched. Implement the rewrite as a resolver-level function in `scripts/agents/agents_config.sh` (e.g. `agents_set_row`) with cases in `test_agents_config.sh` — `agent_admin.sh` stays a thin dispatcher. The rewrite must preserve a row's trailing inline comment (real conf rows carry `# alias: …` comments).
  - **Superseded (2026-07-15):** the *active* set part is no longer how this works — see the Phase 5 retrospective's post-ship note.
...
**Post-ship change (2026-07-15) — row edits infer their family:** As shipped, `agents_set_row` took the family from `[assignments]`, so `/agent <function>.<subtask> <agent>` could only ever edit the *active* family's row and rejected any other family's agent — including this doc's own `/agent cli.commit_prep sonnet` example, which errored because `cli` is on codex. Since the two catalogs share no agent names, the agent argument already determines the family unambiguously, and the assignment lookup was redundant state that only produced spurious errors. `agents_set_row` now derives the family from the agent (`_agents_agent_families_inline`), edits that family's row whether live or dormant, and exports `AGENT_ROW_FAMILY` / `AGENT_ROW_ACTIVE` / `AGENT_ROW_ACTIVE_FAMILY` so `agent_admin.sh` can print a `# updated [<section>] <task> — live|dormant` line. Editing a row still never changes which family is live — only `agents_set_assignment` does. Two guards: an agent listed by *both* catalogs is refused as ambiguous rather than guessed, and an agent whose family has no `[<function>.<family>]` section names that missing section. `agents_list_function` rows gained an `active=yes|no` field (both it and the liveness check share `_agents_active_family`, which honors exact-task assignment overrides). Only `agent_admin.sh` and the tests consume these two functions; `agents_resolve` and every downstream consumer are unchanged.
...
### Phase 7 — clean-fix stage resolution  · status: done (`432ec92`)
...
### Phase 8 — clean-fix driver, usage, and report surfaces  · status: done (`69c79a8`)
...
**What worked:** Seven files landed to spec in one pass; the load-bearing parser-slicing constraint held (stage-start leading phrases byte-identical); the parser generalization carried the family into reason codes (`<family>-usage-limit`) via a suffix-keyed `humanize_cell_reason` and a frozen `AgentLimit` dataclass, with per-family grouping in the run notes.
**What deviated from the plan:** One fix pass (blind reviewer, all three confirmed): the rerouted render's prompt-file `sed` ran unguarded under `set -e` (a missing template would have killed the unattended run — now `if`-guarded, WARN-and-continue); the render log's `.log` name polluted `/clean_fix list`'s `*.log` glob (renamed `report_render.txt`); the durable-limit regex truncated parenthesized reset details at the first `)` (pre-existing, fixed with a greedy end-anchored capture since the phase rewrote that line anyway).
**Surprises:** `report-render.md`'s rendering instructions quote the parser's usage-limit strings verbatim — the parser generalization entailed doc edits beyond the spec'd line 3 (codex caught and handled this unprompted, correctly).
**Implications for remaining phases:** The report render now goes through `agent_exec` under `[assignments] cleanfix=codex` — first scheduled codex-rendered report lands on the next active run (heads-up already recorded; `/agent cleanfix.report …` is the lever). Nothing structural changes for Phases 9-11.

#### Phase 8 Review

- Phase 9's Work Order now pins the consultation launcher's caller-facing protocol (signature `<session_dir> [working_dir]`, `question.md`/`answer.txt`, status `asking→answered|error`) so a fresh session can't normalize it onto the delegate template's names and wedge the doc's poll loop; the call-sites bullet was corrected (only the implementation site gets a new script name — the consult script keeps its name).
- Phase 9 widened: `commands/ask_a_friend.md`'s codex persona strings ("Codex Says", `<AskCodex>`, "Codex implements it") go family-neutral so the doc stays truthful after `/agent ask_a_friend claude` — mechanics untouched (same pattern as Phase 7's durable-line fix).
- Phase 10's session-dir layout no longer names provenance files nothing would write — it now specifies the in-session `agents_resolve_print` capture before launching.
- Phase 11 gained the three stage scripts' stale clean-fix.conf error text (eval 74/77, review 66/69, fix 130/145/148 — line 130 still points users at the removed `agent=` conf key); no other phase owned it.
- Delegation Context refreshed: the four Phase 8 file entries now describe what shipped, and the standing `settings.json` checkpoint exclusion is recorded as an invariant.

### Phase 9 — ask_a_friend migration  · status: done (`e874982`)
...
- `scripts/ask_a_friend/ask_a_friend.sh` (the per-round consultation, called at `commands/ask_a_friend.md` line 88): today reads codex model/effort from the old registry defaults; it becomes status/provenance handling plus `agent_exec ask_a_friend.consultation write …`. It runs write mode (`--full-auto`) deliberately — `--sandbox read-only` panics codex's system-configuration crate on macOS — keep the explanatory comment. Its log becomes `agent.log`; it writes provenance (`task=/family=/agent=/effort=`) to `consult_agent` beside that log. Caller interface unchanged (`commands/ask_a_friend.md` lines 90-93 poll these exact names/strings): signature stays `<session_dir> [working_dir]`; prompt is `${SESSION_DIR}/question.md`; output is `${SESSION_DIR}/answer.txt`; the status file is `${SESSION_DIR}/status` writing `asking` → `answered`|`error` — do not adopt the delegate template's status names. Implementation launcher likewise keeps its protocol: prompt `implementation_prompt.md`, output `impl_summary.txt`, status `impl_status` writing `implementing` → `implemented`|`error`; only the log and provenance file names change.
...
- `commands/ask_a_friend.md`: the implementation call site (line 285) gets the new script name; the consultation call site (line 88) keeps `ask_a_friend.sh` unchanged — only its log ref (line 93) changes to `agent.log` (and line 291 to `impl_agent.log`); the resolution blurb at lines 15-16 ("resolve Codex model/effort defaults through `agents.conf`") rewrites to name the registry rows (`[ask_a_friend.<family>]`, switched via `/agent`). Also neutralize the codex persona strings so the doc stays truthful after a family switch — the doc's purpose line, `<AskCodex>` tag, "Codex Says" headers, and "Codex implements it" phrasing become family-neutral ("your friend" / `<AskFriend>` / "Your friend says"); the doc's mechanics (rounds, history, decision walk) are untouched.
...
**What worked:** Both wrappers landed as exact instances of the Phase 4 template with the pinned ask_a_friend protocol names (`status` `asking→answered|error`, `question.md`/`answer.txt`); the persona neutralization (`<AskFriend>`, "Your Friend") and the resolver-stderr retrofit to all four wrappers shipped in one pass; the blind review itself ran through the retrofitted `review.sh`.
**What deviated from the plan:** One direct doc fix (both reviews agreed): the option-3 alias list lost `"codex"` as a typed shortcut — restored alongside the new `"agent"`.
**Surprises:** The implementation launch died at its tail with `unexpected EOF` exit 2 — the phase edits `scripts/delegate/implement.sh` *while that script is running the launch*, and bash re-reads a self-modified script at a stale byte offset. Harmless here (codex had finished; `impl_status` was already `implemented`), and Phase 4 dodged it only because `mv` preserves the running script's inode. Known hazard: a phase that edits a live launcher in place produces a spurious parse-error exit after the real work completes — check `impl_status`/the diff before treating it as a failure.
**Implications for remaining phases:** No legacy `agents_config_*` callers remain anywhere (ask_a_friend was the last) — Phase 11's verification grep should now come back clean of live code refs before it even starts deleting. Phases 10-11 edit only docs/registry internals, not running launchers, so the self-modification hazard doesn't recur.

#### Phase 9 Review

- Phase 10 gained the operational specifics a fresh session would otherwise guess: per-wave artifact namespacing (`cycle2/`, `adversary/`, `pass3/` — flat names would overwrite across waves), the reviewed project's root as `<working_dir>` (never `~/.claude`), per-task provenance lines, an allow-list entry for direct `agent_exec` launches (added to its Files; Phase 9 precedent), an explicit exception letting the finding-collection sentences change, and an accepted-risk note that claude-family readonly reviewers running the style loader is untested.
...
- Session dirs live under the scratchpad (per-agent `prompt_N.md`/`findings_N.txt`/`agent_N.log`), same layout as delegate sessions. Namespace artifacts per launch wave — `<session_dir>/<wave>/prompt_N.md` etc. (e.g. `cycle2/`, `adversary/`, `pass3/`) — team_review re-launches once per cycle, api_review has two waves, module_review three passes; flat names would overwrite. Provenance: the review commands call `agent_exec` directly (no wrapper writes it), so capture it in-session once before launching — one `bash -c 'source ~/.claude/scripts/agents/agents_config.sh && agents_resolve_print <task>'` line per task the command uses (api_review: `reviewer`+`adversary`; module_review: `reviewer`+`validation`), appended to `<session_dir>/agent_provenance.txt`.
- Working dir: pass the reviewed project's root as `<working_dir>` — `${CRATE_ROOT}` for api_review, the repo root containing `${SCOPE}` for module_review, the project the topic belongs to (typically the session cwd) for team_review; never `~/.claude`. Prompt/output/log paths stay absolute per the `agent_exec` path invariant.
- Permissions: add `"Bash(bash ~/.claude/scripts/agents/agent_exec.sh:*)"` to `.claude/settings.local.json`'s allow list — nothing covers direct `agent_exec` launches today, and each backgrounded reviewer would otherwise prompt (Phase 9 precedent for editing that file).
...
- Everything else in the three docs (dimension menus, finding schema, synthesis, firewall/posture logic, decision walks) is untouched — except the finding-collection sentences (e.g. `team_review.md` ~125 "Collect all findings from returned agents"), which change from "returned agents" to reading each launch wave's findings files.
...
- `.claude/settings.local.json` — one allow-list entry for direct `agent_exec` launches.

**Constraints from prior phases:** Phase 1 rows exist for `team_review.expert`, `api_review.reviewer`, `api_review.adversary`, `module_review.reviewer`, `module_review.validation` in both family sets; Phase 3's `agent_exec` signature and readonly-mode flag mapping are the contract these docs cite — reference the script path `scripts/agents/agent_exec.sh`, don't restate its internals. Accepted risk (note, don't solve): the prompts tell reviewers to run `zsh ~/.claude/scripts/rust_style/load-rust-style.sh` (read-only script) — proven under codex `--sandbox read-only`, untested under claude `--permission-mode plan` with `--print`, which may refuse script execution non-interactively and silently degrade style loading after a family switch; no mechanics change under the shipped codex assignment.
...
**What worked:** All three docs landed with the full shared mechanics (wave namespacing, provenance capture, working-dir rules, unsandboxed warm-up, accepted-risk notes) in one pass; the folded-in operational specifics from the Phase 9 review were exactly what the implementation needed.
**What deviated from the plan:** Two direct doc fixes (both reviews agreed): the new module_review prompt bullet mandated the charter's generic finding schema, contradicting the surviving per-agent format (per-agent format now governs); and no doc said what to do when a backgrounded reviewer fails — all three gained a failure rule (nonzero exit or missing/empty findings → read the agent log, surface, relaunch-or-proceed; never silently an empty review).
**Surprises:** Two blind-review findings were refuted: the "don't restate launcher internals" objection targeted a descriptive sentence that mirrors the plan's own wording (the docs never assemble flags); and pass 2's "each proposed submodule from Agent D" dependency is a pre-existing flaw — parallel in-session subagents couldn't share results either. **Known pre-existing doc flaw (future pass, not this plan):** `module_review.md` pass 2 Agent F's charge references Agent D's proposals, which no parallel launch can deliver — it needs a second wave or a self-derived grouping.
**Implications for remaining phases:** None structural — Phase 11 is docs/registry-internal cleanup, order-independent of this phase.

#### Phase 10 Review

- Phase 11's pre-flight verified everything against the live tree: the full delete list is caller-free repo-wide (wider sweep than the Work Order's grep), the keep list's callers are live, all line refs exact, the verification grep returns exactly the three predicted self-deleting categories, and every acceptance-gate command works post-strip.
...
- Reword the clean-fix.conf stale-key error text in the three stage scripts to match `clean-fix.sh` 205-217 — stage enablement in `agent-assignments.conf`, agent settings in `config/agents.conf`: `style-eval-all.sh` 74/77, `style-eval-review-all.sh` 66/69, `style-fix-worktrees.sh` 130/145/148 (line 130 still says "use enabled=true|false and agent=claude|codex", directing users to a conf key Phase 7 removed). Also `style-eval-review-all.sh` line 43's header comment ("Agent assignment comes from agent-assignments.conf" — false since Phase 7): reword to say agent assignment resolves from `config/agents.conf` via the registry; `agent-assignments.conf` carries only `enabled=`.
...
- `scripts/clean-fix/style-eval-all.sh`, `style-eval-review-all.sh`, `style-fix-worktrees.sh` — stale-key error text reworded (see Spec).
