...
If the exported pending markdown has no `## Fix Summary`, do **not** review the current pending findings directly. This means the normal handoff is stale or incomplete: the evaluation/review stage exists, but the fix agent's report was not saved into pending JSON.
...
eval_path="$recovery_path"
printf 'recovered_eval_path=%s\n' "$eval_path"
```

Then read `eval_path` and continue the review from the recovered markdown. The recovered file either wraps the old scratch fix evaluation if it still exists, or reconstructs the finding list from the latest style history row with finding-like guideline outcomes. In the history-row case, the original finding locations and the original Fix Summary prose may be unavailable; review the current worktree diff directly against the recovered style files and the recorded outcomes.

When Recovered Handoff Mode is used, the first user-facing response must start with a short `## Handoff Recovery` section that says:

- this is an error state, not the normal path
- pending JSON was stale or incomplete and lacked `## Fix Summary`
- the review is being salvaged from the recovered source named in `eval_path`
- the current diff is being evaluated against recovered style rules, so any missing original location/search detail is explicitly unknown
...
**STEP 1:** Execute `<ReadEvaluation/>` — start with pending markdown when it contains `## Fix Summary`; otherwise enter Recovered Handoff Mode and use the recovered evaluation file.
