...
The clone's directory (`bevy-${VERSION}`) lives under `~/rust/` but is never evaluated, reviewed, or fixed by the fix pipeline: that conf is an opt-in allowlist, so the upstream Bevy source is skipped simply by not being listed in `[projects]`. (The permissions back-population step still visits every directory under `~/rust/`.) No action needed here.
