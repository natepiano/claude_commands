---
description: Attach a live Monitor to a running clean-fix clean/build/style-eval/style-fix log and report state transitions as they happen
---

# Monitor Clean-fix

Attach a persistent <MonitorTool/> to whichever clean-fix-related script the user just kicked off (clean+rebuild, style evaluation, style-fix worktrees, or the full clean-fix orchestrator) and surface meaningful state transitions in real time. Skip the high-volume noise (SKIP lines, raw cargo output) — only emit lines the user would act on.
...
- If `$ARGUMENTS` is non-empty but the file does not exist, inform the user: `Log not found: $ARGUMENTS. Run /monitor_clean_fix with no argument to autodetect, or pass a valid path.` Then stop.
...
- **No fresh candidates** — inform the user: `No clean-fix logs modified in the last 2 hours. Pass an explicit path: /monitor_clean_fix <path>.` Then stop.
