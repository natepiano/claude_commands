...
<RevertCleanFixRedirect>
    ...
    - The helper drops any `[active_checkout]` redirect pointing into the worktree; it is a no-op if there was none. With `--commit` it commits **only** `clean-fix.conf` in the `~/.claude` repo when something changed, so the conf needs no manual upkeep.
