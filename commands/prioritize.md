...
6. **Write the approved stars directly** to the issue frontmatter: only the five judgment fields, as double-quoted YAML star scalars (e.g. `backlog_alignment: "⭐⭐⭐⭐"`). Never write `backlog_score` or `backlog_rank` — the watcher owns them and recomputes on its own, in about 3-4 seconds end to end even when the edit renumbers the whole backlog. Wait for `last-status` to reach `ok ... result=updated` before verifying; do not poll the issue files in a tight loop. If a re-rank takes tens of seconds, `ProcessType` in the watcher's launchd plist has regressed to `Background` — see the comment in `com.natemccoy.hanadocs-prioritize.plist`.
