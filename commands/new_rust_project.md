...
- **Member** — report the member path. It is already built, formatted, enrolled in the fix pipeline, and committed to the workspace repo. Tell the user to review and push when ready. Do **not** suggest `/add_github_repo` — a member has no repo of its own.
</RunScript>
