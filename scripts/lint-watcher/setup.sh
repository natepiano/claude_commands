#!/bin/bash
# Setup script for the lint watcher launchd agent.
# Idempotent — safe to run multiple times. Only acts on what's missing.

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PLIST_NAME="com.natemccoy.lint-watcher.plist"
...
chmod +x "$SCRIPT_DIR/lint-watcher.sh" "$SCRIPT_DIR/run-lint.sh"

# 4. Symlink plist into ~/Library/LaunchAgents/
...
# 5. Load the launchd agent if not already loaded
